...
export default async function DetailPage({
  ...
  return (
    <Container className="py-8 md:py-12 relative z-10">
      ...
      <div className="relative w-full h-[380px] md:h-[480px] rounded-3xl overflow-hidden mb-8 border border-white/10 shadow-2xl bg-violet-950">
        {backdropPath ? (
          <Image
            src={backdropPath}
            alt={title}
        ...
        <div className="absolute bottom-6 left-6 right-6 flex flex-col md:flex-row items-end md:items-center justify-between gap-4">
          <div className="space-y-2 max-w-2xl">
            {tagLine && (
              <span className="inline-flex items-center gap-1.5 text-xs font-black text-amber-400 bg-amber-500/10 border border-amber-500/30 px-3 py-1 rounded-full uppercase tracking-wider">
            ...
            <div className="flex items-center gap-3 text-xs md:text-sm font-extrabold text-zinc-300 flex-wrap">
              <span className="flex items-center gap-1 text-amber-400">
                <Star className="w-4 h-4 fill-current" /> {rating} ({voteCount} votes)
              </span>
              <span>•</span>
              <span className="flex items-center gap-1">
                <Calendar className="w-4 h-4 text-violet-400" /> {releaseDate.substring(0, 4)}
              </span>
              {runtime && (
                <>
                  <span>•</span>
                  <span className="flex items-center gap-1">
                    <Clock className="w-4 h-4 text-violet-400" /> {runtime}
                  </span>
                </>
              )}
              <span>•</span>
              <span className="bg-violet-600/40 text-violet-200 border border-violet-500/40 px-2.5 py-0.5 rounded-md text-xs">
                {status}
              </span>
            </div>
          </div>
          ...
          {trailer && (
            <a
              ...
              className="px-6 py-3 rounded-2xl bg-violet-600 hover:bg-violet-500 text-sm font-black text-white shadow-xl shadow-violet-600/40 transition-all flex items-center gap-2 flex-shrink-0 active:scale-95"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-4 space-y-6">
          <div className="relative aspect-[2/3] w-full rounded-2xl overflow-hidden border border-white/10 shadow-2xl bg-violet-950">
            <Image
              src={posterPath}
              alt={title}
              fill
        ...
        <div className="lg:col-span-8 space-y-6">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              {genres.map((g: any) => (
                <Link
                  ...
                  className="bg-white/5 hover:bg-violet-600/30 border border-white/10 hover:border-violet-500/40 px-3.5 py-1.5 rounded-xl text-xs font-extrabold text-zinc-300 hover:text-white transition-all"
                >
                  {g.name}
                </Link>
              ))}
          ...
          {/* Synopsis Plot */}
          <div className="bg-[#14142f] border border-white/8 rounded-2xl p-6 space-y-3 shadow-xl">
            <h3 className="text-lg font-black text-white">Storyline & Plot Summary</h3>
          ...
          {/* Key Details Card */}
          <div className="bg-[#14142f] border border-white/8 rounded-2xl p-6 space-y-4 shadow-xl">
            <h3 className="text-lg font-black text-white">Production Details</h3>
      ...
      {/* Recommended Reviews Section (Detailed Horizontal Cards Layout matching Pic 2!) */}
