...
export default function DetailTrailerButton({ videoKey, title, mediaType = 'movie', mediaId }: DetailTrailerButtonProps) {
  if (mediaId) {
    return (
      <a
        href={`https://movies.xflix.ink/watch/${mediaType}/${mediaId}`}
        target="_blank"
      ...
      >
        <Play className="w-3.5 h-3.5 sm:w-4 sm:h-4 fill-current" /> Watch Official Trailer & Full HD Movies Here!
      </a>
    );
  }
